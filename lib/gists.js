var settings = require("../settings");
var github = require("./github");

var when = require("when");
var fs = require("fs");
var path = require("path");

var mongojs = require('mongojs');

var db = mongojs(settings.mongo.url,["flows","users","tags"]);

var GIST_DIR = path.join(__dirname,"..","gists");

if (!fs.existsSync(GIST_DIR)) {
    fs.mkdirSync(GIST_DIR);
}


function calculateRatings(gist) {
    if (typeof gist.ratings === 'undefined'
            || gist.ratings.length <= 0) {
        return 0;
    }
    else {
        var r = gist.ratings;
        var sum = 0;
        for (var i=0; i<r.length; i++) {
            sum += parseInt(r[i].rating);
        }
        return Math.round((sum/gist.ratings.length)*2)/2;
    }
}

function addToSearchIndex(id,doc) {
    var si = require('search-index');
    var d = {};
    d[id] = doc;

    // console.log(id, doc)
    // si.del(id, function(msg) {
    //     si.add({'batchName': 'gist',
    //             'filters': ['tags']}, d, function(msg) {});
    // });

    si.deleteDoc(id, function(msg) {
        //console.log(d)
        si.index(JSON.stringify(d), '', ['tags'], function(msg) {});
    });
    
}

function removeFromSearchIndex(id) {
    var si = require('search-index');
    // si.del(id, function(msg) {
    //     console.log();
    // });
    si.deleteDoc(id, function(msg) {
        console.log();
    });
}


function getGist(id,projection) {
    projection = projection || {};
    var defer = when.defer();
    db.flows.findOne({_id:id},projection,function(err,data) {
        if (err||!data) {
            defer.reject();
        } else {
            defer.resolve(data);
        }
    });
    return defer.promise;
}


function refreshGist(id) {
    var defer = when.defer();
    getGist(id,{etag:1,tags:1,added_at:1}).then(function(gist) {
        github.getGist(id,gist.etag).then(function(data) {
            if (data == null) {
                // no update needed
                db.flows.update({_id:id},{$set: {refreshed_at:Date.now()}},function(err) {
                    if (err) {
                        console.log(err);
                    }
                    defer.reject(true);
                });
            } else {
                var p = path.join(GIST_DIR,id);
                fs.readdir(p,function(err,files) {
                    for (var i=0;i<files.length;i++) {
                        fs.unlinkSync(path.join(p,files[i]));
                    }
                    fs.rmdirSync(p);
                    data.added_at = gist.added_at;
                    data.tags = gist.tags;
                    defer.resolve(addGist(data));
                });
            }
        }).otherwise(function(err) {
            removeGist(id).then(function() {
                defer.reject(false);
            });
        });
    }).otherwise(function() {
        defer.reject(false);
    });
    
    return defer.promise;
}

function ensureUserExists(login) {
    var defer = when.defer();
    console.log("ensureUserExists:",login);
    db.users.findOne({_id:login},function(err,user) {
        if (user) {
            console.log(" - exists");
            defer.resolve();
        } else {
            console.log(" - not found");
            github.getUser(login).then(function(data) {
                data._id = data.login;
                db.users.save(data);
                defer.resolve();
            }).otherwise(function(err) {
                defer.reject(err);
            });
        }
    });
    return defer.promise;
}

function createGist(accessToken,gist,tags) {
    var defer = when.defer();
    github.createGist(gist,accessToken).then(function(data) {
        for (var i=0;i<tags.length;i++) {
            db.tags.update({_id:tags[i]},{$inc:{count:1}},{upsert:true});
        }
        data.added_at = Date.now();
        data.tags = tags;
        defer.resolve(addGist(data));
    }).otherwise(function(err) {
        console.log("ERROR",err);
        defer.reject(err);
    });
    return defer.promise;
}

function addGist(data) {
    var defer = when.defer();
    if (!data.files['flow.json']) {
        defer.reject("Missing file flow.json");
        return;
    }
    if (!data.files['README.md']) {
        defer.reject("Missing file README.md");
        return;
    }
    fs.mkdir(path.join(GIST_DIR,data.id),function(err) {
        if (err) {
            defer.reject(err);
        } else {
            var files = {};
            var readme = "";
            for (var fn in data.files) {
                var file = data.files[fn];
                var ffn = path.join(GIST_DIR,data.id,fn);  
                files[fn.replace(/\./g,"-")] = ffn;
                fs.writeFileSync(ffn,file.content,'utf8');
                if (fn == "README.md") {
                    readme = file.content;
                }
            }
            data.files = files;
            data.refreshed_at = Date.now();
            data._id = data.id;
            data.ratings = [];
            data.ratings_avg = 0;
            db.flows.save(data,function(err,other) {
                if (err) {
                    console.log(err,other);
                }
            });
            // this is where the gist fields returned by search queries are built
            var doc = {
                ratings_avg: String(data.ratings_avg),
                tags:data.tags,
                description:data.description,
                readme:readme,
                created_at: data.created_at,
                updated_at: data.updated_at,
                comments: String(data.comments),
                owner__login: data.owner.login,
                owner__avatar_url: data.owner.avatar_url,
            };
            addToSearchIndex(data.id,doc);
            
            ensureUserExists(data.owner.login).then(function() {
                defer.resolve(data.id);
            }).otherwise(function() {
                defer.reject();
            });
        }
    });
    return defer.promise;
}

function addGistById(id) {
    console.log("Add gist [",id,"]");
    var defer = when.defer();
    github.getGist(id).then(function(data) {
        data.added_at = Date.now();
        data.tags = [];
        defer.resolve(addGist(data));
    }).otherwise(function(err) {
        defer.reject(err);
    });
    return defer.promise;
}

function removeGist(id) {
    var defer = when.defer();
    getGist(id).then(function(gist) {
        for (var i=0;i<gist.tags.length;i++) {
            db.tags.update({_id:gist.tags[i]},{$inc:{count:-1}});
        }
        db.tags.remove({count:{$lte:0}});

        var p = path.join(GIST_DIR,id);
        fs.readdir(p,function(err,files) {
            if (err) { 
                defer.resolve();
            } else {
                for (var i=0;i<files.length;i++) {
                    fs.unlinkSync(path.join(p,files[i]));
                }
                fs.rmdirSync(p);
                db.flows.remove({id:id});
                removeFromSearchIndex(id);
                defer.resolve();
            }
        });
    });
    return defer.promise;
}

function getGists(query, orderBy) {
    var defer = when.defer();
    if (typeof orderBy === 'undefined') {
        orderBy = {added_at:-1}; //default order by most recent
    }
    db.flows.find(
        {
            $query: query,
            $orderby:orderBy,
        },
        {
            id: 1,
            description: 1,
            'owner.login': 1,
            'owner.avatar_url': 1,
            created_at: 1,
            updated_at: 1,
            tags: 1,
            comments: 1,
            ratings_avg: 1,
        },
        function(err,gists) {
            // //calculate ratings
            // for(var i=0; i<gists.length; i++) {
            //     var r = calculateRatings(gists[i]);
            //     gists[i].ratings_avg = r;
            // }
            defer.resolve(gists);
        });
    
    return defer.promise;
}

function getGistsForUser(userId) {
    return getGists({'owner.login':userId});
}
function getGistsForTag(tag) {
    return getGists({tags:tag});
}
function getAllGists(test) {
    return getGists({});
}

function getUser(id) {
    var defer = when.defer();
    db.users.findOne({_id:id}, function(err,user) {
        if (user == null) {
            defer.reject();
        } else {
            defer.resolve(user);
        }
    });
    return defer.promise;
}

function updateTags(id,tags) {
    tags = tags||[];
    var defer = when.defer();
    getGist(id,{ ratings_avg: 1,
                 tags:1,
                 description:1,
                 comments: 1,
                 created_at: 1,
                 updated_at: 1,
                 'files.README-md':1,
                 'owner.login':1,
                 'owner.avatar_url':1,
                }
    ).then(function(gist) {

        var oldTags = gist.tags;
        
        if (oldTags.length == tags.length) {
            var matches = true;
            for (var i=0;i<oldTags.length;i++) {
                if (tags.indexOf(oldTags[i]) == -1) {
                    matches= false;
                    break;
                }
            }
            if (matches) {
                defer.resolve();
                return;
            }
        }
        
        for (var i=0;i<oldTags.length;i++) {
            if (tags.indexOf(oldTags[i]) == -1) {
                db.tags.update({_id:oldTags[i]},{$inc:{count:-1}});
            }
        }
        for (var i=0;i<tags.length;i++) {
            if (oldTags.indexOf(tags[i]) == -1) {
                db.tags.update({_id:tags[i]},{$inc:{count:1}},{upsert:true});
            }
        }
        db.tags.remove({count:{$lte:0}});

        db.flows.update({_id:id},{$set: {tags:tags}},function(err) {
            if (err) {
                defer.reject(err);
            } else {
                fs.readFile(gist.files['README-md'],{encoding:'utf8'},function(err,readme) {
                    if (!err) {
                        var doc = {
                            ratings_avg: String(gist.ratings_avg),
                            tags:gist.tags,
                            description:gist.description, //this is actually the title
                            readme:readme,
                            created_at: gist.created_at,
                            updated_at: gist.updated_at,
                            comments: String(gist.comments),
                            owner__login: gist.owner.login,
                            owner__avatar_url: gist.owner.avatar_url
                        };
                        addToSearchIndex(id,doc);
                    }
                    defer.resolve();
                });
            }
        });
        
    }).otherwise(function(err) {
        console.log(err);
        defer.reject(err);
    });
    return defer.promise;
}

function updateRatings(id, rating, uid) {
    var defer = when.defer();

    if (typeof rating === 'undefined') {
        defer.reject('missing rating');
    }
    getGist(id,{ ratings: 1,
                 tags:1,
                 description:1,
                 comments: 1,
                 created_at: 1,
                 updated_at: 1,
                 'files.README-md':1,
                 'owner.login':1,
                 'owner.avatar_url':1,
                }
    ).then(function(gist) {
        var r = gist.ratings || [];
        var rated = false;

        // check if there is already an entry for the user
        for (var i=0;i<r.length;i++) {
            if (r[i].uid === uid) {
                r[i].rating = rating;
                rated = true;
            }
        }
        if (rated === false) {
            // add new rating entry
            r.push({rating: rating, uid: uid})
        }
        gist.ratings = r;
        gist.ratings_avg = calculateRatings(gist);

        db.flows.update({_id:id}, 
                { $set: {ratings:r, 
                         ratings_avg:  gist.ratings_avg}
                }, function(err) {
            if (err) {
                defer.reject(err);
            }
            else {
                fs.readFile(gist.files['README-md'],{encoding:'utf8'},function(err,readme) {
                    if (err) {
                        defer.reject(err);
                    }
                    else {
                        var doc = {
                            ratings_avg: String(gist.ratings_avg),
                            tags:gist.tags,
                            description:gist.description, //this is actually the title
                            readme:readme,
                            created_at: gist.created_at,
                            updated_at: gist.updated_at,
                            comments: String(gist.comments),
                            owner__login: gist.owner.login,
                            owner__avatar_url: gist.owner.avatar_url
                        };
                        addToSearchIndex(id,doc);
                        defer.resolve(gist);
                    }
                });
            }
        });
    }).otherwise(function(err){
        console.log(err);
        defer.reject(err);
    });
    return defer.promise;
}


function getTags(query) {
    var defer = when.defer();
    //TODO: add limit for getting tags
    db.tags.find({$query:query,$orderby:{count:-1,_id:1}},function(err,tags) {
        defer.resolve(tags);
    });
    
    return defer.promise;
}
function getAllTags() {
    return getTags({});
}
    
module.exports = {
    add: addGistById,
    refresh: refreshGist,
    remove: removeGist,
    updateTags: updateTags,
    updateRatings: updateRatings,
    get: getGist,
    getAll: getAllGists,
    getGists: getGists,
    getForUser: getGistsForUser,
    getUser: getUser,
    create: createGist,
    getAllTags: getAllTags,
    getForTag: getGistsForTag
}

//var repo = "https://gist.github.com/6c3b201624588e243f82.git";
//var sys = require('sys');
//var exec = require('child_process').exec;
//function puts(error, stdout, stderr) { sys.puts(stdout); sys.puts(stderr);  }
//exec("git clone "+repo, puts);
//


